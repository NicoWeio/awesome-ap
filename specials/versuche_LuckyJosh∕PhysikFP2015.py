...
def getVersuchNummer(string):
    s = re.search(r'V\s*(\d{1,3})', string, re.IGNORECASE)
    if s:
...
def analyze(path):
    try:
        ...
        search_result = re.search(r'\\newcommand{\\ENo}{(.*)}', main_tex_content)
